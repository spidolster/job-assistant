...
from modules.storage import save_resume, get_saved_resumes, get_resume_path
...
st.title("🕵️‍♂️ Job Assistant — Resume Analyzer")
st.markdown("Analisis kecocokan resume Anda dengan Job Description menggunakan AI.")
...
with col1:
    ...
    resume_options = ["📤 Upload Resume Baru"] + saved_resumes
...
with col2:
    ...
    jd_text = st.text_area("Paste teks lengkap Job Description di sini", height=300)
...
if st.button("🚀 Analisis Kecocokan", use_container_width=True, type="primary"):
    ...
    if not api_key:
        ...
        # Extract resume text
        with st.spinner("Membaca resume..."):
            if selected_resume_option == "📤 Upload Resume Baru":
                ...
                with st.spinner("Menyimpan resume ke history..."):
                    saved_filename = save_resume(uploaded_file, custom_name="")
                    # Kita tidak st.rerun() di sini agar proses analisis tetap lanjut
        ...
        if not resume_text:
            st.error("❌ Gagal membaca teks dari PDF. Pastikan file tidak diproteksi atau berupa gambar.")
        else:
            # Analyze
